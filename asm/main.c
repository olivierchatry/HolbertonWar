...
void generate_instruction_code(generator_t* generator, opcode_t* opcode, char** output, int count) {
...
void generate_instruction(generator_t* generator, char** output, int count) {
	opcode_t* opcode = holberton_core_get_opcodes();
	while (opcode->mnemonic) {
		if (strcmp(opcode->mnemonic, *output) == 0) {
			break;
		}
		opcode++;
	}
	if (opcode->mnemonic) {
		if ((count - 1) != opcode->arg_count) {
			...
			generate_instruction_code(generator, opcode, output + 1, count - 1);
		}
	}
	else {
...
void generate_code(generator_t* generator, char* hex_string) {
	char*	start = NULL, *end = NULL;
	while (*hex_string) {
		if (*hex_string != ' ' && start == NULL) {
			start = hex_string;
...
int generate_handle_label(generator_t* generator, char** output, int count) {
	int offset = 0;
...
void generate(generator_t* generator, char** output, int count) {
	if (strcmp(output[0], ".name") == 0) {
		strncpy(generator->core.name, output[1], CORE_FILE_NAME_MAX_SIZE - 1);
	} else if (strcmp(output[0], ".extended") == 0) {

	} else if (strcmp(output[0], ".comment") == 0) {
		strncpy(generator->core.comment, output[1], CORE_FILE_COMMENT_MAX_SIZE - 1);
	} else if (strcmp(output[0], ".code") == 0) {
		generate_code(generator, output[1]);
	} else {
		int offset = 0;
		offset = generate_handle_label(generator, output, count);

		if (offset < count) {
			generate_instruction(generator, output + offset, count - offset);
		}
	}
}
...
void write_output_file(char* input_file_name, generator_t* generator) {
	...
	if (!output_file_name) {
		...
		int32	output_size = generator->byte_code_offset;
		...
		if (output == NULL) {
			...
			generator->core.magic 		= generator_int32(generator, CORE_FILE_MAGIC);
			generator->core.code_size = generator_int32(generator, generator->core.code_size);
			fwrite(&generator->core, 1, sizeof(generator->core), output);
			fwrite(generator->byte_code + generator->byte_code_base, 1, output_size, output);
			fclose(output);
		}
	}
}
  ...
  FILE* 				input;
	...
	if (input) {
		...
		while (!feof(input) && generator.error == ASM_OK) {
			...
			if (count > 0) {
				generate(&generator, output, count);
			}
			else if (count < 0) {
